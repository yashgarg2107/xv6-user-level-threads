// first test case for second part of assignment (Synchronization)
...
void t_func1(void)
{
  ...
  lock_busy_wait_acquire(lock1);    // critical section begins
  ...
  lock_busy_wait_release(lock1);    // critical section ends
  thread_schedule();
}
...
void t_func2(void)
{
  lock_busy_wait_acquire(lock2);    // critical section begins
  ...
  lock_busy_wait_release(lock2);    // critical section ends
